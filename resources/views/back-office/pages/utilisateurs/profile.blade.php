				...
				<div class="wideget-user text-center">
					<div class="wideget-user-desc">
						...
						<div class="user-wrap">
							...
							<a href="{{route('regenerepassword', $user->uuid)}}" class="btn btn-primary mt-1 mb-1">Régénérer mot de passe</a>						</div>
					</div>
				</div>
			</div>
		</div>
					...
					<div class="card-body">
						<div id="profile-log-switch">
							...
							<div class="text-center">
								...
								<a   data-toggle="tooltip" data-placement="top" title="Supprimer ce compte" onClick="event.preventDefault(); deleteConfirm('{{ $user->uuid }}')" class="btn btn-danger btn-sm ml-2" href="{{route('delete-user',$user->uuid)}}"><i class="fa fa-trash"></i></a>
                                            <form id="{{ $user->uuid }}" action="{{route('delete-user',$user->uuid)}}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                            </form>
								@endif
